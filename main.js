...
var CTRL = {
	...
	init:		function(){
        ...
        var views = new Array(		
			...
			new Viewport(document.getElementById('cnv-NE'))
			//new Viewport(document.getElementById('cnv-NE'))
			//new Viewport(document.getElementById('cnv-SE')),
			//new Viewport(document.getElementById('cnv-SW')),
		);
		...
		KeyboardController({
			// MODEL ROTATION
			81:		function(){ CTRL.currview().transforms.z += 0.2 },
			69: 	function(){ CTRL.currview().transforms.z -= 0.2 },
			87: 	function(){ CTRL.currview().transforms.y += 0.2 },
			65: 	function(){ CTRL.currview().transforms.y -= 0.2 },
			83: 	function(){ CTRL.currview().transforms.x -= 0.2 },
			68: 	function(){ CTRL.currview().transforms.x += 0.2 },
			// CAMERA CONTROLS
			37: 	function(){ CTRL.currview().actvcam().x += 0.025; },
			38: 	function(){ CTRL.currview().actvcam().y += 0.025; },
			39: 	function(){ CTRL.currview().actvcam().x -= 0.025; },
			40:		function(){ CTRL.currview().actvcam().y -= 0.025; },
			109: 	function(){ CTRL.currview().actvcam().z -= 0.05; },
			107: 	function(){ CTRL.currview().actvcam().z += 0.05; },
			88: 	function(){ console.log('Next camera.'); CTRL.currview().nextcam(); },
			90: 	function(){ console.log('Previous camera.'); CTRL.currview().prevcam(); },
		...
		}, {81: 1, 69: 1, 87: 1, 83: 1, 65:1, 68:1, 37:1, 38:1, 39:1, 40:1, 32:0,  109:1,  107:1, 88: 0, 90: 0}, function(){});
		...
		//CTRL.viewlist[0].buffer.push(sphere);
		//CTRL.viewlist[1].buffer.push(octhed);
		CTRL.viewlist[0].buffer.push(tSphere);
		CTRL.viewlist[1].buffer.push(cubeOb);
		//CTRL.viewlist[4].buffer.push(qSphere);
